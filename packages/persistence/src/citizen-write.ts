...
export interface CreateCitizenInput {
  ...
  backstory?: string; goal?: string; worldId?: string;
}
...
export async function createCitizen(input: CreateCitizenInput): Promise<void> {
  ...
  await pool.query(
    `INSERT INTO citizens (id,name,occupation,age,traits,wealth,reputation,tier,created_day,world_id)
     VALUES ($1,$2,$3,$4,$5,0,50,$6,$7,$8) ON CONFLICT (id) DO NOTHING`,
    [input.id, input.name, input.occupation, input.age, JSON.stringify(input.traits), input.tier, input.createdDay, input.worldId ?? "genesis"]);
  if (input.backstory) {
    await pool.query(
