...
TRACK I (Task 17, web /explain route): DEFERRED — explicitly OPTIONAL in plan ("only if CLI acceptance passes + time permits"). CLI acceptance is met and all 4 invariants proven; the web view is not part of the substrate thesis or the Phase 1A definition of done. Clean additive follow-up (route + keyless read API + render test reusing CausalChain/SocialDrivers); does not block the PR. Controller decision.
