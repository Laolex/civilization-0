...
TRACK E IMPLEMENTATION COMPLETE (schema + append + persistTick wiring). Tip: e602a0a.

TRACK E CONSOLIDATED REVIEW (sonnet, a7de6d1..e602a0a): Changes required → ALL FIXED (commit d00d44e). Affirmed correct: Inv#1 (no fabrication), Inv#2 transactional coupling (client passed, catch{ROLLBACK} unwinds both, rollback itest sound), Inv#4 (schemaVersion stored+read as-is), JSONB↔canonicalJSON hash round-trip stable (pg sorts jsonb keys), RLS deny-all + owner-bypass matches posture.
  CRITICAL (FIXED): append tip-read held no lock → 2 concurrent ticks same world both read same tip, both append same parentHash → silent chain fork, verifyChain then breaks undetectably (Inv#3). Verified caller (scheduler/src/loop.ts:45-53) ticks citizens SEQUENTIALLY today so fork is LATENT not active — but added airtight guard anyway: UNIQUE INDEX history_world_parent_uq (world_id,parent_hash) makes fork structurally impossible (loser hits UNIQUE → whole tick rolls back, Inv#2) + FOR UPDATE on tip read for graceful blocking on the common non-genesis path. Index verified present in civ0_test.
  IMPORTANT (FIXED): repository.ts worldId fell back to "default" if citizen row missing → transition appended to phantom chain unreconcilable to any real decision. Now throws (rolls back whole tick) on missing world_id.
  MINOR (FIXED): history-append.itest stale comment claimed resetWorld doesn't truncate history (it does now) + removed the redundant DELETE.
  Re-verified: typecheck 14 projects clean; history unit 30/30; append itest 1/1; FULL persistence integration suite 46/46 across 23 files.
TRACK E COMPLETE. Tip: d00d44e.

Task 13: complete (commit 6c1791e, controller inline). verify.ts (verifyWorldChain = verifyChain over loaded CognitiveTransition rows; faithfulnessProof folds shadow vs legacy decisions, WARN-ONLY) + read.ts (loadLegacyActions, decisions⋈citizens.world_id) + warn-only proof wired post-COMMIT in persistTick on this.pool with own try/catch (never fails/rolls back a committed tick). Integration 4/4 (untampered chain ok; tamper via jsonb_set→ok false; faithful match→ok+[]; ADDED divergence case→ok false + {folded:rest,legacy:work}). Full persistence 46/46 (no divergence warnings on normal tick since selectedAction==d.action). typecheck clean.
  TDD NOTE (honest): verify.ts/read.ts written alongside the test, so a separate RED was not captured for these. Mitigant: the suite carries genuine failure-path assertions (tamper→ok:false, divergence→specific {folded,legacy}) that are non-vacuous and would fail wrong logic. Whole-branch review to independently scrutinize verify.ts.
TRACK F COMPLETE (verification proofs). Tip: 6c1791e.

Task 14: complete (commit e13eec7, controller inline, RED captured then GREEN). read.ts +loadTransition (latest-wins scan, filters CognitiveTransition kind) +loadAnchor (history_anchors latest by created_at); explainView.ts buildExplainView (load→verifyWorldChain→loadAnchor→project explain); scripts/explain.ts CLI (9-step trace, arg parser). Integration 2/2 (reconstructs verified trace + null→'unavailable'; missing→null); typecheck clean incl scripts/.
  ★ PHASE 1A ACCEPTANCE MET (end-to-end, live vs civ0_test): seeded a real tick then ran the actual CLI `tsx scripts/explain.ts --citizen c1 --tick 5 --world we` → printed: "chain verified : ✓", "④ candidates : unavailable", "⑦ beliefΔ : unavailable", full authenticated 9-step trace folded from the log, anchor "unanchored". The definition of done for Phase 1A is satisfied. (seed row cleaned from civ0_test afterward — shared parentHash=GENESIS for 'we' would otherwise collide with history_world_parent_uq on the next itest run.)
TRACK G COMPLETE (civ explain CLI = acceptance surface). Tip: e13eec7. Everything below (Track H anchor, Track I web) is ADDITIVE — must not regress the CLI or the live tick.

Task 15: complete (commit e92a65b, controller inline, RED→GREEN). anchor.ts anchorTick (filter tick's CognitiveTransition rows → merkleRoot → storage.archive civ.history/v0/<world>/<tick> best-effort → INSERT history_anchors ON CONFLICT upsert → append AnchorEvent; null if no transitions). Integration 2/2 (anchors w/ fakeStorage → 0xROOT/0xTX + anchor row + 1 kind='Anchor' event; empty tick → null). Wired gated best-effort anchor into persistTick post-commit (HISTORY_ANCHOR=1, this.pool, createZeroGStorage(loadZeroGConfig(process.env)), own try/catch, OFF by default). typecheck clean; persistence 46/46 with anchor gated off.
Task 16: complete (commit 29207f1). scripts/anchor-smoke.ts manual gated live-0G script (uses real loadZeroGConfig, not plan's placeholder). NOT executed — spends ~0.002 OG, manual-only by design. typecheck clean incl scripts/.
TRACK H COMPLETE (0G anchor — additive optimization layer, gated OFF). Tip: 29207f1.

TRACK I (Task 17, web /explain route): COMPLETE (commit b63a001's successor; controller inline, RED→GREEN). Built AFTER Phase 1A acceptance + whole-branch review were already done, on user request ("start work on task 17", keep branch local). Files: apps/web/app/explain/ExplainPanel.tsx (presentational client cmp — reuses shipped SocialDrivers, renders the 9-step trace + chain-verified/broken badge; Inv#1: candidates/beliefΔ "unavailable" === literal render, never fabricated), apps/web/app/explain/[citizen]/[tick]/page.tsx (server page, sync params, buildExplainView→ExplainPanel or empty-state), apps/web/app/api/explain/[citizen]/[tick]/route.ts (keyless read, ?world= default "default", 400 invalid tick / 404 not found / 502 error, runtime=nodejs force-dynamic), apps/web/app/explain/explain.test.tsx (jsdom render test, 3 cases).
  ADAPTED from plan's generic stubs to shipped conventions: getPool from @civ/persistence/src/pool (not "@civ/persistence"), relative component imports (no @/ alias), Next 14.2.5 SYNC params (matches citizens/[id]/page.tsx), ExplainPanel colocated at app/explain/ so the test's ./ExplainPanel import is honest (plan put it under [citizen]/[tick]). Tightened one test assertion to a data-testid (availableActions+selectedAction both contain "work" → getByText(/\bwork\b/) would throw on multiple matches).
  WIRING: @civ/history added to apps/web/package.json deps (workspace:*) + pnpm install — web tsconfig resolves @civ/* via workspace symlinks (moduleResolution Bundler), not the base path map, so the dep declaration is required. (Pre-existing history↔persistence cyclic-workspace warning is from repository.ts dynamic imports, unrelated; web is not in that cycle.)
  VERIFIED: explain test 3/3; FULL web suite 66/66 across 19 files (no regression); pnpm -r typecheck clean (14 projects). NOT booted via next dev — getPool() would hit the live civ0 DB the scheduler ticks; render test + typecheck cover the contract. Live engine + CLI untouched (purely additive route).

WHOLE-BRANCH REVIEW (opus, independent, bfee34d..29207f1): Ready to merge WITH FIXES → all must-fix + 2 related minors FIXED (commit 68170a1). Affirmed clean: Task7 engine change behavior-neutral (only live-path change; availableActions=forced??ALL_ACTIONS identical value; new TickResult fields are pure reads), Inv#1 no-fabrication, Inv#2 same-tx coupling + post-commit proof/anchor can't corrupt committed tick, JSONB value-round-trip hashing stable, AnchorEvent shape handled by readers, schema idempotency/RLS, explain.ts null-safety.
  CRITICAL (FIXED): verifyWorldChain filtered to CognitiveTransition only, but append() builds ONE chain over ALL kinds — once an AnchorEvent interleaves (…T,Anchor,T…), the filtered subset gaps the linkage → honest anchored chain false-reports "BROKEN" the moment HISTORY_ANCHOR=1 (and buildExplainView→civ explain prints ✗ BROKEN). Fix: verifyWorldChain now walks the FULL chain (verifyChain(rows)); AnchorEvent parentHash/eventHash + header-envelope cross-check round-trip cleanly. Added regression itest (transition→anchor→transition → ok:true); PROVEN to have teeth (RED against the old filter, GREEN with fix).
  MINOR (FIXED): (2) re-anchoring threw on duplicate event_id while the anchors row upserted — anchorTick now skips the AnchorEvent append if its deterministic id already exists (re-anchorable, no throw, no chain dup). (3) FOR UPDATE comment overstated the guarantee on the bare-Pool path — reworded to credit the UNIQUE(world_id,parent_hash) index as the airtight backstop.
  MINOR (DEFERRED, forward-looking, noted not fixed): (4) faithfulnessProof + verifyWorldChain are O(n) over full world history per tick / per explain — warn-only & correct, but cost grows on the live 2h scheduler; revisit with windowed/incremental verification before scale. (5) Invariant #4 dispatch machinery absent — schemaVersion is stamped on every event but readers read raw v1; the version-branching reader must land before any v2 event is emitted (no violation in 1A, only v1 exists).
  Re-verified after fixes: unit 237/237 (57 files), integration 63/63 (31 files, +1 regression), typecheck clean (14 projects).
PHASE 1A IMPLEMENTATION + REVIEW COMPLETE. Code tip: 68170a1. → superpowers:finishing-a-development-branch (PR; NEVER merge to master directly).
